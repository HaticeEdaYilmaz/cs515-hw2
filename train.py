...
def get_transforms(params, train=True):
    ...
    if params["pretrained"] and params["option"] == 1:
    ...
    if resize:
        tf_list.append(transforms.Resize((resize, resize)))

    if train:
        if not resize:  # only do random crop for small images
            tf_list.append(transforms.RandomCrop(32, padding=4))
